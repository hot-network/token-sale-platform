...
// Fix: Import Modal component directly to resolve 'Cannot find name' error.
...
// ---- HELPER & CHILD COMPONENTS FOR SOCIAL SHARING ----
const SocialShareButtons: React.FC<{ shareText: string; shareUrl: string }> = ({ shareText, shareUrl }) => {
    const links = {
    ...
    const buttonClass = "w-10 h-10 flex items-center justify-center rounded-lg text-gray-500 dark:text-gray-400 hover:text-brand-accent dark:hover:text-brand-accent hover:bg-gray-200 dark:hover:bg-brand-dark-lighter transition-all";

    return (
        <div className="flex items-center justify-center gap-3">
            <a href={links.twitter} target="_blank" rel="noopener noreferrer" className={buttonClass} aria-label="Share on X">
                <i className="fa-brands fa-x-twitter text-xl"></i>
            </a>
            <a href={links.facebook} target="_blank" rel="noopener noreferrer" className={buttonClass} aria-label="Share on Facebook">
                <i className="fa-brands fa-facebook-f text-xl"></i>
            </a>
            <a href={links.telegram} target="_blank" rel="noopener noreferrer" className={buttonClass} aria-label="Share on Telegram">
...
// ---- HELPER & CHILD COMPONENTS FOR ACTIVE SALE ----
...
const ActiveSaleTransactionHistory: React.FC<{ transactions: Transaction[] }> = ({ transactions }) => {
    const { networkConfig } = useTokenSalesContext();
    const [isOpen, setIsOpen] = useState(true);
    const listId = 'transaction-history-list-active';
        ...
        <div>
            ...
            >
                {transactions.length === 0 ? (
                    <p className="text-xs text-gray-500 text-center py-4">Your recent transactions will appear here.</p>
                ) : (
...
const BuyForm: React.FC<{ stage: SaleStageConfig; }> = ({ stage }) => {
    ...
    return (
        <div className="space-y-3">
            ...
            <Button onClick={handleBuyClick} disabled={isButtonDisabled} className="w-full text-lg py-3 cursor-pointer bg-brand-accent text-brand-dark hover:brightness-105 active:brightness-95">
                {buttonText}
            </Button>
...
const ActiveSalePanel: React.FC<{ stage: SaleStageConfig; countdown: Countdown; totalSold: number; }> = ({ stage, countdown, totalSold }) => {
    // Fix: Add 'transactions' to destructuring to make it available in the component scope.
    const { networkConfig, user, isDripping, dripFaucetTokens, transactions } = useTokenSalesContext();

    const shareUrl = typeof window !== 'undefined' ? window.location.href : '';
    const shareText = `I'm participating in the HOT Token presale on Solana! Join me and be part of the future of decentralized content. #HOTtoken #Solana #Presale`;
        ...
        <div className="p-4 sm:p-6 min-h-[550px]">
            <div className="grid grid-cols-1 lg:grid-cols-2 lg:gap-8">
                ...
                <div className="mt-8 lg:mt-0">
                    <div className="bg-brand-light-dark/50 dark:bg-brand-dark-light/50 rounded-xl p-4 sm:p-6 h-full">
                        <ActiveSaleTransactionHistory transactions={transactions.history} />
                        <div className="text-center pt-6 mt-6 border-t border-gray-200 dark:border-gray-700">
                            <p className="text-gray-600 dark:text-gray-400 mb-4 text-sm font-semibold">Help spread the word!</p>
                            <SocialShareButtons shareText={shareText} shareUrl={shareUrl} />
                        </div>
                    </div>
                </div>
            </div>
...
// ---- HELPER & CHILD COMPONENTS FOR OTHER SALE STATES ----

const EndedSaleTransactionHistory: React.FC<{ transactions: Transaction[] }> = ({ transactions }) => {
    const { networkConfig } = useTokenSalesContext();
    const [isOpen, setIsOpen] = useState(transactions.length > 0);
    const listId = 'transaction-history-list-ended';
...
const UpcomingSalePanel: React.FC<{ stage: SaleStageConfig; countdown: Countdown }> = ({ stage, countdown }) => {
    const shareUrl = typeof window !== 'undefined' ? window.location.href : '';
    const shareText = `Join the HOT Token presale on Solana! Don't miss out on the future of decentralized content. #HOTtoken #Solana #Presale`;
...
const EndedSalePanel: React.FC = () => {
    ...
    const shareUrl = typeof window !== 'undefined' ? window.location.href : '';
    const shareText = "The HOT Token presale has ended! I'm excited for what's next for the HOT Network. #HOTtoken #Solana";
            ...
            <div className="py-8 px-4 flex flex-col items-center justify-center min-h-[460px]">
                 ...
                 <div className="w-full max-w-sm mt-auto">
                    <div className="mt-8 text-center border-t border-gray-200 dark:border-gray-700 pt-6">
                        <p className="text-sm font-semibold text-gray-600 dark:text-gray-400 mb-3">Share your participation!</p>
                        <SocialShareButtons shareText={shareText} shareUrl={shareUrl} />
                    </div>
                    <EndedSaleTransactionHistory transactions={transactions.history} />
                </div>
            </div>
            {/* Fix: Remove invalid <lazy> tag and render Modal component directly. */}
            {isClaimModalOpen && (
                <Modal 
